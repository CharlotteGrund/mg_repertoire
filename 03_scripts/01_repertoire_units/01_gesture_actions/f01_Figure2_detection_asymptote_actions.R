...
# Figure 2. Detection asymptote for gesture actions --------
...
# select relevant variables (record, gesture action)
xx <- xx %>% select(Recording_number, Gesture_record) 

# convert gesture_record (getsure action) to factor if it's not already
xx$Gesture_record <- as.factor(xx$Gesture_record)

# empty vector to store cumulative counts
cumulative_counts <- numeric(length(levels(xx$Gesture_record)))
...
for (i in 1:nrow(xx)) {
  ...
  cumulative_counts[xx$Gesture_record[i]] <- 1
...
asymptote_index <- min(which(cumulative_unique_levels == 63))
asymptote_index <- 1530

# gesture action asymptoe plot
ggplot(plot_data, aes(x = Cumulative_Recording_Number, y = Cumulative_Unique_Gesture_Levels)) +
  ...
  annotate("text", x = cumulative_recording_numbers[asymptote_index], y = max(plot_data$Cumulative_Unique_Gesture_Levels) * 0.9,
           label = "1530", color = "black", hjust = 1, vjust = 1, angle = 90, size = 6) +  # Increase size of label
  labs(x = "Gesture tokens",
       y = "Repertoire size (GAs)") +
  theme_minimal() +
  scale_x_continuous(expand = c(0, 0), limits = c(-100, 3500 * 1.1)) +  # Adjust x-axis limits
  scale_y_continuous(expand = c(0, 0), limits = c(-1, 65 * 1.1)) +  # Adjust y-axis limits
